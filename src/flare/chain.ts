...
export async function getPStakesOf(network: string, pAddress: string, stakes?: Array<PStake>): Promise<Array<PStake>> {
  if (!stakes) {
...
export async function getPStakesTo(network: string, pAddress: string, stakes?: Array<PStake>): Promise<Array<PStake>> {
  if (!stakes) {
    stakes = await getPStakes(network);
  }
  let nodeIds = stakes
...
async function _parsePStake(
  network: string,
  stake: CurrentDelegatorData | CurrentValidatorData,
  type: string
): Promise<PStake> {
  let address: string;
  if (
    "validationRewardOwner" in stake &&
