...
export async function moveCP(
  ...
  if (exportParams.network !== importParams.network || exportParams.publicKey !== importParams.publicKey) {
...
export async function _checkNodeId(account: Account, nodeId: string, stakes?: Array<PStake>): Promise<void> {
  if (!stakes) {
...
async function _waitForStake(account: Account, txId: string): Promise<void> {
  await utils.waitWhile(
    async () => (await chain.getPStakesOf(account.network, account.pAddress)).some((x) => x.txId === txId),
    TX_EFFECT_TIMEOUT,
    TX_EFFECT_DELAY
  );
}
