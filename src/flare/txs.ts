...
async function _getExportCTxFee(account: Account, amount: BN, importFeeReservation: BN): Promise<BN> {
...
export async function buildImportCTx(account: Account, params: ImportCTxParams): Promise<UnsignedTxData> {
...
async function _getUnsignedImportCTx(account: Account, importFee: BN): Promise<UnsignedTx> {
  ...
  return evm.newImportTx(context, cAddress, [pAddress], utxos, context.pBlockchainID, BigInt(importFee.toString()));
}

export async function buildImportPTx(account: Account, params: ImportPTxParams): Promise<UnsignedTxData> {
  const context = await getContext(account.network);
  ...
  if (isEtnaForkActive) {
    ...
    unsignedTx = pvm.newImportTx(context, context.cBlockchainID, utxos, [pAddress], [pAddress]);
  }
...
export async function buildExportPTx(account: Account, params: ExportPTxParams): Promise<UnsignedTxData> {
  const context = await getContext(account.network);
  const pvmapi = new pvm.PVMApi(settings.URL[account.network]);
  const feeState = await pvmapi.getFeeState();
  const pAddressString = account.pAddress;
  ...
  const output = TransferableOutput.fromNative(context.avaxAssetID, BigInt(amount.toString()), [pAddress]);
  ...
  if (isEtnaForkActive) {
    ...
    unsignedTx = pvm.newExportTx(context, context.cBlockchainID, [pAddress], utxos, [output]);
  }
  const unsignedTxHex = _unsignedTxToHex(unsignedTx);
  return {
...
export async function buildAddDelegatorTx(account: Account, params: DelegatorPTxParams): Promise<UnsignedTxData> {
  const context = await getContext(account.network);
  const pvmapi = new pvm.PVMApi(settings.URL[account.network]);
  const feeState = await pvmapi.getFeeState();
  const pAddressString = account.pAddress;
...
export async function buildAddValidatorTx(account: Account, params: ValidatorPTxParams): Promise<UnsignedTxData> {
  const context = await getContext(account.network);
  const pvmapi = new pvm.PVMApi(settings.URL[account.network]);
  const feeState = await pvmapi.getFeeState();
  const pAddressString = account.pAddress;
...
export async function buildBaseTx(account: Account, params: TransferPTxParams): Promise<UnsignedTxData> {
  const context = await getContext(account.network);
  const pvmapi = new pvm.PVMApi(settings.URL[account.network]);
  const feeState = await pvmapi.getFeeState();
  const pAddressString = account.pAddress;
    ...
    unsignedTx = pvm.e.newBaseTx(
      {
        ...
        utxos,
        outputs: [TransferableOutput.fromNative(context.avaxAssetID, BigInt(params.amount), [recipientAddressBytes])],
      },
      context
    );
  } else {
    unsignedTx = pvm.newBaseTx(context, [pAddressBytes], utxos, [
      TransferableOutput.fromNative(context.avaxAssetID, BigInt(params.amount), [recipientAddressBytes]),
    ]);
  }
  const unsignedTxHex = _unsignedTxToHex(unsignedTx);
  return {
